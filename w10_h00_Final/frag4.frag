...
vec3 circle ( float posX, float posY, float rad, vec3 color){
    ...
    float r = length(vec2(posX,posY))*20.0;
...
void main()
{
    ...
    // st1= length(pos)*(st*3.);
    float noise = 10.0 *  -.1 * turbulence( 1.1 * vec3(st.x,st.y,u_time/1200.) + u_time/1000.);
    ...
    vec2 pos = vec2(mouseX,mouseY)-st1;
    vec3 color1 = vec3(.5,.8,.1);
    vec3 c1 = vec3(0.);
    

    c1 = circle( pos.x*noise,pos.y*noise,.1,color1)*9.*noise;

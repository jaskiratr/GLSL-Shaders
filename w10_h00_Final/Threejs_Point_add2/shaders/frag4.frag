...
void main()
{
    ...
    float r =1.;
    float a  = 1.;
    float mouseX =  u_mouse.x*3.;
    float mouseY =  3.-u_mouse.y*3.;

    vec2 uv = st*3.;
    // vec2 pos = vec2(mouseX,mouseY)-uv;
    // vec2 pos2 = vec2((u_points[1].x,u_points[1].y)) - uv;
    
    float noise = 10.0 *  -.1 * turbulence( 1. * vec3(uv.x,uv.y,u_time/200.) + u_time/1000.);
    ...
    for(int i = 0; i<20; i++){
      if (u_points[i].x != 0.){
        vec2 pos = vec2((u_points[i].x,u_points[i].y)) - uv;
        r *= length(pos)*20.0*noise;
        a *= atan(pos.y,pos.x);
